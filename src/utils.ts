...
function isOutOfContainer(parentRef: React.RefObject<HTMLElement>, childRef: React.RefObject<HTMLElement>) {
  ...
  if (parentBound && childBound && (parentBound.top - childBound.top < 0 || parentBound.left - childBound.left < 0 || parentBound.right - childBound.right < 0 || parentBound.bottom - childBound.bottom < 0)) return false;
  return true;
}
